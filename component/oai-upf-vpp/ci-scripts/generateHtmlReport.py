...
class HtmlReport():
	...
	def buildSummaryHeader(self):
		...
		self.file.write('		<th>OAI UPF-VPP <font color="Gold">Ubuntu20</font> Image</th>\n')
	...
	def analyze_docker_clone_patch(self, nfType):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				...
				section_end_pattern = 'make: Entering directory .*/vpp-upf/vpp'
				section_status = False
				vppClone = False
				upgClone = False
				patching = False
				...
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						result = re.search('FROM vpp-upf-base:latest', line)
						if result is not None:
							usingBaseImage = True
						result = re.search(section_start_pattern, line)
						if result is not None:
							section_status = True
							res2 = re.search('^#([0-9]+).* RUN', line)
							if res2 is not None:
								build_stage_id = res2.group(1)
						...
						if result is not None and section_status:
							section_status = False
							status = vppClone and upgClone and patching
						result = re.search('Cloning into \'vpp\'...', line)
						if result is not None and section_status:
							vppClone = True
						result = re.search('Cloning into \'upg-vpp\'...', line)
						if result is not None and section_status:
							upgClone = True
						result = re.search('patching file src/vnet/devices/tap/tap.c', line)
						if result is not None and section_status:
							patching = True
					logfile.close()

				if usingBaseImage:
					cell_msg = '      <td bgcolor="LightSeaGreen"><pre style="border:none; background-color:LightSeaGreen"><b>'
					cell_msg += 'N/A:\n'
					cell_msg += ' -- Not performed</b></pre></td>\n'
				elif usingCache:
					cell_msg = '      <td bgcolor="DarkSeaGreen"><pre style="border:none; background-color:DarkSeaGreen"><b>'
					cell_msg += 'N/A:\n'
					cell_msg += ' -- Not performed by using the CACHE</b></pre></td>\n'
				elif status:
					cell_msg = '      <td bgcolor="LimeGreen"><pre style="border:none; background-color:LimeGreen"><b>'
					cell_msg += 'OK:\n'
					cell_msg += ' -- All Git Operations went successfully</b></pre></td>\n'
	...
	def analyze_install_log(self, nfType):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				...
				section_start_pattern = 'RUN git clone -b stable/2101 https://github.com/fdio/vpp.git'
				if variant == 'docker':
					section_end_pattern = 'COPY --from=vpp-upf-builder'
				else:
				...
				if status:
					...
					cell_msg = '	  <td bgcolor="Tomato"><pre style="border:none; background-color:Tomato"><b>'
					cell_msg += 'KO:\n'
				if not self.git_pull_request and not usingBaseImage:
	...
	def analyze_copy_conf_tool_log(self, nfType):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				...
				if variant == 'docker':
					section_end_pattern = 'naming to docker.io/library/oai-upf-vpp:'
				else:
	...
	def analyze_image_size_log(self, nfType, imageKind):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				if nfType == 'UPF-VPP':
					if variant == 'docker':
						section_start_pattern = 'naming to docker.io/library/oai-upf-vpp:'
				...
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						...
						if section_status:
							if nfType == 'UPF-VPP':
								result = re.search(f'oai-upf-vpp *{imageTag}', line)
